...
function requireOwner(i) { 
  if (i.user.id !== SUPREME_OWNER_ID) {
    throw new Error('Access Denied: Only the Supreme Owner of Astrix can execute this command.');
  }
}
...
function isBlacklisted(guildId, userId) {
  if (userId === SUPREME_OWNER_ID || premium.isOwner(userId)) return false;
  const s = state();
...
async function command(i, client) {
  ...
  if(group==='access'){
    ...
    if(sub==='blacklistuser'){
      const user=i.options.getUser('user',true);
      if (user.id === SUPREME_OWNER_ID) throw new Error('You cannot blacklist the Supreme Owner.');
      addUnique(st.blacklistedUsers, user.id);
      store.save();

      // Automatically send a direct message notification to the user
      try {
        const fetchedUser = await client.users.fetch(user.id);
        if (fetchedUser) {
          await fetchedUser.send(`⚠️ **ASTRIX™ SECURITY NOTICE** ⚠️\nYou are restricted to use Astrix all cmds.`);
        }
      } catch (dmErr) {
        console.error(`Could not send restriction DM to user ${user.id}:`, dmErr);
      }

      return i.reply({embeds:[ui.ownerEmbed('Owner • User Access',`${user} → **Blacklisted**`)],ephemeral:true});
    }

    if(sub==='unblacklistuser'){
      const user=i.options.getUser('user',true);
      remove(st.blacklistedUsers, user.id);
      store.save();
      return i.reply({embeds:[ui.ownerEmbed('Owner • User Access',`${user} → **Removed from blacklist**`)],ephemeral:true});
    }

    // Astrix Global Ban & Unban subcommands
    if(sub==='astrixban'){
      const user=i.options.getUser('user',true);
      if (user.id === SUPREME_OWNER_ID) throw new Error('You cannot ban the Supreme Owner from Astrix.');
      addUnique(st.blacklistedUsers, user.id);
      store.save();
      ...
      try {
        ...
        console.error(`Could not send Astrix ban DM to user ${user.id}:`, dmErr);
      }

      return i.reply({embeds:[ui.ownerEmbed('Owner • Astrix Global Ban',`${user} has been globally banned and restricted from using all Astrix commands.`)],ephemeral:true});
    }
    ...
    if(sub==='astrixunban'){
      ...
      remove(st.blacklistedUsers, user.id);
      store.save();
      return i.reply({embeds:[ui.ownerEmbed('Owner • Astrix Global Unban',`${user} has been unbanned and restored to use Astrix commands.`)],ephemeral:true});
    }

    const gid=i.options.getString('guild_id',true); 
    resolveGuild(client,gid); 
    
    if(sub==='blacklistserver'){
      addUnique(st.blacklistedGuilds, gid);
    } else {
      remove(st.blacklistedGuilds, gid);
    }
    store.save();
